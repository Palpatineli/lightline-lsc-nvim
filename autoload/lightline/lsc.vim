...
let s:indicator_warnings = get(g:, 'lightline#lsc#indicator_warnings', 'W:')
...
function! lightline#lsc#errors() abort
    ...
    let l:error_no = len(filter(getqflist(), 'v:val["type"] == "E"'))
    return l:error_no == 0 ? '' : printf(s:indicator_errors . " %d", error_no)
endfunction

function! lightline#lsc#warnings() abort
    if (LanguageClient_serverStatus() == 1)
        return ''
    endif
    let l:num = len(filter(getqflist(), 'v:val["type"] == "W"'))
    return l:num == 0 ? '' : printf(s:indicator_warnings . " %d", num)
endfunction
